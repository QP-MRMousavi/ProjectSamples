...
async function generateMainReportExcel(
  req: NextApiRequest,
  res: NextApiResponse
) {
  const tmp = req.query;
  const reqData: MainAbsenceReportRequest = {
    startingDate: tmp["sd"] ? new Date(tmp["sd"] as string) : undefined,
    endingDate: tmp["ed"] ? new Date(tmp["ed"] as string) : undefined,
  };
  console.log("log generateMainReportExcel reqData", reqData);
  const data: UserActivitiesPerWeekResults = await getTimeSheetDetails(reqData);
